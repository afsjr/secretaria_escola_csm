...
export async function ProfessorView(
  ...
  // =====================================================
  const renderMinhasDisciplinas = (): string => {
    if (errorDisciplinas) {
      return `<p class="error-text">Erro ao carregar disciplinas.</p>`;
    }
    if (!disciplinas || disciplinas.length === 0) {
      return "<p>Você não possui disciplinas vinculadas.</p>";
    }

    // Agrupar disciplinas por período
    const disciplinasPorPeriodo: Record<string, DisciplinaData[]> = {};
          ...
          <div style="background: white; padding: 1.5rem; border-radius: var(--radius-lg); box-shadow: var(--shadow-sm);">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;">
              <h3 style="margin: 0; color: var(--text-main);">
                <span style="color: var(--primary);">Período:</span> ${
        escapeHTML(periodo)
      }
              </h3>
  ...
  // =====================================================
  const renderLancarNotas = (): string => `
    <div id="painel-notas">
      <div style="background: white; padding: 1.5rem; border-radius: var(--radius-lg); box-shadow: var(--shadow-sm);">
  ...
  // =====================================================
  const renderRegistroAulas = (): string => `
    <div id="painel-aulas">
      <div style="background: white; padding: 1.5rem; border-radius: var(--radius-lg); box-shadow: var(--shadow-sm);">
        <div style="margin-bottom: 1.5rem;">
  ...
  btnsContarAlunos.forEach((btn) => {
    btn.addEventListener("click", async () => {
      const turmaId = (btn as HTMLButtonElement).getAttribute("data-turma");
      (btn as HTMLButtonElement).disabled = true;
      (btn as HTMLButtonElement).textContent = "...";

      const { count, error } = await ProfessorService.contarAlunosTurma(
        turmaId!,
      ) as { count: number; error: { message: string } | null };

      if (error) {
        toast.error("Erro ao contar alunos: " + error.message);
        (btn as HTMLButtonElement).textContent = "Erro";
      } else {
        (btn as HTMLButtonElement).textContent = `${count} aluno(s)`;
      }

      setTimeout(() => {
        (btn as HTMLButtonElement).disabled = false;
        (btn as HTMLButtonElement).textContent = "Ver Alunos";
      }, 3000);
    });
  });
  ...
  // =====================================================
  const selectDisciplinaNotas = container.querySelector(
    "#select-disciplina-notas",
  ) as HTMLSelectElement;
  const tabelaNotasContainer = container.querySelector(
    "#tabela-notas-container",
  ) as HTMLElement;

  selectDisciplinaNotas.addEventListener("change", async () => {
    const selectedOption =
      selectDisciplinaNotas.options[selectDisciplinaNotas.selectedIndex];
    ...
    if (error) {
      tabelaNotasContainer.innerHTML =
        `<p class="error-text">Erro ao carregar alunos: ${error.message}</p>`;
      return;
    }
      ...
      alunosComNotas.map((a) => `
        ...
        escapeHTML(a.aluno_email)
                ...
                <td style="text-align: center;">
                  <input type="number" class="input faltas-input" min="0" value="${
        a.nota?.faltas || 0
  ...
  // =====================================================
  const selectDisciplinaAulas = container.querySelector(
    "#select-disciplina-aulas",
  ) as HTMLSelectElement;
  const tabelaAulasContainer = container.querySelector(
    "#tabela-aulas-container",
  ) as HTMLElement;

  selectDisciplinaAulas.addEventListener("change", async () => {
    const selectedOption =
      selectDisciplinaAulas.options[selectDisciplinaAulas.selectedIndex];
    ...
    const { data: aulas, error } = await ProfessorService.getAulasDaDisciplina(
      disciplinaId,
    ) as { data: AulaData[] | null; error: { message: string } | null };

    if (error) {
      tabelaAulasContainer.innerHTML =
        `<p class="error-text">Erro ao carregar aulas: ${error.message}</p>`;
      return;
    }
      ...
      <div style="background: #f8fafc; padding: 1.5rem; border-radius: 8px; margin-bottom: 1.5rem; border: 1px solid var(--secondary);">
        ...
        <form id="form-nova-aula" style="display: flex; gap: 1rem; align-items: flex-end; flex-wrap: wrap;">
          ...
          <div class="form-group" style="flex: 0; margin: 0; min-width: 140px;">
            ...
            <input type="date" id="aula-data" name="data" class="input" value="${
    ...
    const formNovaAula = tabelaAulasContainer.querySelector(
      "#form-nova-aula",
    ) as HTMLFormElement;
    if (formNovaAula) {
      formNovaAula.addEventListener("submit", async (e: Event) => {
        e.preventDefault();
        ...
        const btn = formNovaAula.querySelector(
          'button[type="submit"]',
        ) as HTMLButtonElement;
        btn.disabled = true;
        ...
        if (error) {
          ...
          formNovaAula.reset();
          (container.querySelector("#aula-data") as HTMLInputElement).value =
            new Date().toISOString().split("T")[0];
          // Recarregar aulas
    ...
    btnsRemoverAula.forEach((btn) => {
      btn.addEventListener("click", async () => {
        const aulaId = (btn as HTMLButtonElement).getAttribute("data-id");

        if (
          !window.confirm(
            "Certeza que deseja remover esta aula? Esta ação não pode ser desfeita.",
          )
        ) return;
        (btn as HTMLButtonElement).disabled = true;
        (btn as HTMLButtonElement).textContent = "...";
  ...
  btnsLancarNotas.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      tabBtns.forEach((b) => b.classList.remove("active"));
      (container.querySelector('[data-tab="notas"]') as HTMLElement).classList
        .add("active");
      tabContents.forEach((content) =>
        (content as HTMLElement).style.display = "none"
      );
      (container.querySelector("#tab-notas") as HTMLElement).style.display =
        "block";
  ...
  btnsRegistrarAula.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      tabBtns.forEach((b) => b.classList.remove("active"));
      (container.querySelector('[data-tab="aulas"]') as HTMLElement).classList
        .add("active");
      tabContents.forEach((content) =>
        (content as HTMLElement).style.display = "none"
      );
      (container.querySelector("#tab-aulas") as HTMLElement).style.display =
        "block";

      // Selecionar a disciplina no dropdown
      const disciplinaId = (btn as HTMLButtonElement).getAttribute("data-id")!;
