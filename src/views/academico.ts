...
import { createOption, escapeHTML } from "../lib/security";
...
export async function AcademicoView(
  ...
  const { data: profiles, error } = await getAllProfiles() as any;
  if (error) {
    console.error("Erro ao buscar perfis:", error);
    toast.error(
      'Erro de conexão com o banco de dados. Verifique a tabela "perfis".',
    );
  }
  const students =
    profiles?.filter((p: any) =>
      !p.perfil || p.perfil.toLowerCase() !== "admin"
    ) || [];

  // Fetch disciplinas from database
  let modulosFromDB: any = null;
  try {
    const { data: allDisciplinas } = await CourseService.getDisciplinasDoCurso(
      null,
    ) as any;
    if (allDisciplinas && allDisciplinas.length > 0) {
      // Group by modulo
      const grouped: Record<string, string[]> = {};
      allDisciplinas.forEach((d: any) => {
    ...
    console.warn(
      "Erro ao carregar disciplinas do banco, usando fallback:",
      err,
    );
  }
  ...
  container.innerHTML = `
    <header style="margin-bottom: 2rem;">
      <h1 style="font-size: 2rem; color: var(--text-main);">${
    isAdmin ? "Controle Acadêmico" : "Boletim Escolar"
  }</h1>
      <p>${
    isAdmin
      ? "Gerencie as notas e presenças dos alunos por módulo. (Ambiente do Professor/Gestor)"
      : "Espelho das suas notas e presenças. (Apenas Leitura)"
  }</p>
    </header>
      ...
      <div style="display: flex; gap: 1rem; align-items: flex-end;">
        <div class="form-group" style="flex: 1; margin: 0;">
          ...
          <select id="aluno-select" name="aluno_select" class="input">
            ...
            ${
        students.map((s: any) =>
          createOption(s.id, `${s.nome_completo} (${s.email})`)
        ).join("")
      }
          </select>
        </div>
    ...
    <div id="boletim-container" style="${
    isAdmin ? "display: none;" : "display: block;"
  }">
      <div style="display: flex; justify-content: flex-end; margin-bottom: 1rem; gap: 0.5rem;">
        <button id="print-boletim-btn" class="btn btn-primary" style="display: ${
    isAdmin ? "none" : "flex"
      ...
      ${
    modulos.map((modulo: any, mIdx: number) => `
        <div style="background: white; border-radius: var(--radius-lg); box-shadow: var(--shadow-sm); overflow: hidden; margin-bottom: 2rem;">
          ...
          <div style="overflow-x: auto;">
            <table style="width: 100%; border-collapse: collapse; text-align: left; min-width: 800px;">
              ...
              <tbody>
                ${
      modulo.disciplinas.map((disciplina: string, dIdx: number) => {
  ...
  // Logic
  const select = container.querySelector("#aluno-select") as HTMLSelectElement;
  const loadBtn = container.querySelector(
    "#load-student-btn",
  ) as HTMLButtonElement;
  const boletimContainer = container.querySelector(
    "#boletim-container",
  ) as HTMLElement;
      ...
      try {
        ...
        const turmaInfo = matricula?.turmas
          ? {
            ...
            curso_nome: matricula.turmas.cursos?.nome ||
        ...
        container.querySelectorAll(".disciplina-row").forEach((row) => {
          const disciplina = (row as HTMLElement).getAttribute(
            "data-disciplina",
          );
        ...
        const doc = await PDFService.generateBoletimPDF(
          alunoData,
          notasData,
          turmaInfo,
          { marcaCopia: true },
        ...
        PDFService.downloadPDF(
          doc,
          `boletim_${
            (alunoData.nome_completo || "aluno").replace(/\s+/g, "_")
          }.pdf`,
        );
  ...
  async function refreshGradesUI(alunoId: string) {
    if (!alunoId) return;
    const { data, error } = await AcademicService.getBoletim(alunoId) as any;
    if (error) {
      toast.error("Falha na rede ao puxar notas: " + error.message);
      return;
    }

    // Clear ghost values
    container.querySelectorAll(".disciplina-row input").forEach((i) =>
    ...
    if (data && data.length > 0) {
      data.forEach((dbRow: any) => {
        const row = container.querySelector(
          `tr[data-disciplina="${dbRow.disciplina}"]`,
        ) as HTMLElement;
        if (row) {
          const faltasInput = row.querySelector(
            ".faltas-input",
          ) as HTMLInputElement;
          faltasInput.value = dbRow.faltas || "";
          const notaInputs = row.querySelectorAll(".nota-input") as NodeListOf<
            HTMLInputElement
          >;
          notaInputs[0].value = dbRow.n1 || "";
          notaInputs[1].value = dbRow.n2 || "";
          notaInputs[2].value = dbRow.n3 || "";
    ...
    container.querySelectorAll(".nota-input").forEach((i) =>
      (i as HTMLInputElement).dispatchEvent(ev)
    );
    container.querySelectorAll(".rec-input").forEach((i) =>
      (i as HTMLInputElement).dispatchEvent(ev)
    );
  ...
  if (isAdmin) {
    ...
    saveBtn.addEventListener("click", async () => {
      ...
      saveBtn.textContent = "Salvando Nuvem...";
      ...
      if (error) {
        toast.error("Falha de Segurança (Erro ao salvar): " + error.message);
      } else toast.success("Boletim lacrado e salvo com sucesso!");

      saveBtn.disabled = false;
      saveBtn.textContent = "Salvar Registros";
    });
  } else {
  ...
  if (!isAdmin) {
    allInputs.forEach((input) => {
      ...
      (input as HTMLInputElement).style.background = "#f9fafb";
  ...
  rows.forEach((row) => {
    const notasInputs = row.querySelectorAll(".nota-input") as NodeListOf<
      HTMLInputElement
    >;
    const mediaTeoriaCell = row.querySelector(".media-teoria") as HTMLElement;
    const recInput = row.querySelector(".rec-input") as HTMLInputElement;
    const mediaFinalCell = row.querySelector(".media-final") as HTMLElement;
    ...
    const calculateGrades = () => {
      ...
      if (mediaTeoria !== null) {
        ...
        mediaTeoriaCell.style.color = mediaTeoria >= 7
          ? "var(--success)"
          : "var(--danger)";
      } else {
      ...
      if (!isNaN(recVal) && mediaTeoria !== null) {
        ...
        mediaFinalCell.style.color = final >= 7
          ? "var(--success)"
          : "var(--danger)";
      } else if (mediaTeoria !== null) {
        mediaFinalCell.textContent = mediaTeoria.toFixed(1);
        mediaFinalCell.style.color = mediaTeoria >= 7
          ? "var(--success)"
          : "var(--danger)";
      } else {
