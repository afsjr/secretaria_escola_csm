...
export async function AuditLogView() {
  ...
  const { data: logs, error, count } = await AuditService.getLogs({ limit: 50 })
  ...
  const sc = severityCounts || { alta: 0, media: 0, baixa: 0 }
  ...
  const tableContainer = container.querySelector('#audit-log-table-container')

  btnFiltrar.addEventListener('click', async () => {
    const acao = container.querySelector('#filtro-acao').value
    const severidade = container.querySelector('#filtro-severidade').value
    ...
    if (severidade) {
      filteredLogs = filteredLogs.filter(l => ACTION_SEVERITY[l.acao] === severidade)
    }

    // Filtro por busca textual
    const busca = container.querySelector('#filtro-busca').value.toLowerCase()
    if (busca) {
  ...
  btnLimpar.addEventListener('click', () => {
    container.querySelector('#filtro-acao').value = ''
    container.querySelector('#filtro-severidade').value = ''
  ...
  if (btnLoadMore) {
    ...
    btnLoadMore.addEventListener('click', async () => {
      ...
      const { data: moreLogs, count: totalCount } = await AuditService.getLogs({ limit: 50, offset })
...
function renderLogRow(log) {
  ...
  const initials = log.usuario_nome ? escapeHTML(log.usuario_nome.charAt(0).toUpperCase()) : '?'
    ...
    <tr style="cursor: pointer;" title="${escapeHTML(log.descricao || '')}"
      ...
      <td>
        <div style="display: flex; align-items: center; gap: 0.5rem;">
          ...
          <div>
            <div style="font-weight: 600; font-size: 0.85rem;">${escapeHTML(log.usuario_nome || 'Desconhecido')}</div>
            <div style="font-size: 0.7rem; color: var(--text-muted);">${escapeHTML(log.usuario_perfil || '—')}</div>
          </div>
        </div>
      </td>
      <td>
